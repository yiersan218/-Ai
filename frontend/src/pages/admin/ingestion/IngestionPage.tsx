...
const CHUNK_STRATEGY_OPTIONS = [
  { value: "fixed_size", label: "fixed_size（固定大小）" },
  { value: "structure_aware", label: "structure_aware（结构感知）" }
];

const ENHANCER_TASK_OPTIONS = [
  { value: "context_enhance", label: "context_enhance（上下文增强）" },
  { value: "keywords", label: "keywords（关键词提取）" },
  { value: "questions", label: "questions（问题生成）" },
  { value: "metadata", label: "metadata（元数据提取）" }
];

const ENRICHER_TASK_OPTIONS = [
  { value: "keywords", label: "keywords（关键词提取）" },
  { value: "summary", label: "summary（摘要生成）" },
  { value: "metadata", label: "metadata（元数据添加）" }
];
...
const NODE_CONDITION_GUIDES: Record<PipelineNodeType, { placeholder: string; emptyHelp: string }> = {
  fetcher: {
    placeholder: [
      '本地文件：{"field":"source.type.value","operator":"eq","value":"file"}',
      'URL：{"field":"source.type.value","operator":"eq","value":"url"}',
      '飞书：{"field":"source.type.value","operator":"eq","value":"feishu"}'
    ].join("\n"),
    emptyHelp: "留空时，本地文件、URL 和飞书三种来源都会执行当前节点。"
  },
  parser: {
    placeholder: [
      '原始内容存在：{"field":"rawBytes","operator":"exists"}',
      'PDF 文档：{"field":"mimeType","operator":"eq","value":"application/pdf"}',
      '指定文件名：{"field":"source.fileName","operator":"eq","value":"example.pdf"}'
    ].join("\n"),
    emptyHelp: "留空时，所有已获取的文档都会执行解析节点。"
  },
  enhancer: {
    placeholder: [
      '解析正文存在：{"field":"rawText","operator":"exists"}',
      'PDF 文档：{"field":"mimeType","operator":"eq","value":"application/pdf"}',
      '指定元数据存在：{"field":"metadata[department]","operator":"exists"}'
    ].join("\n"),
    emptyHelp: "留空时，所有已解析的文档都会执行增强节点。"
  },
  chunker: {
    placeholder: [
      '增强正文存在：{"field":"enhancedText","operator":"exists"}',
      '解析正文存在：{"field":"rawText","operator":"exists"}',
      '已生成关键词：{"field":"keywords[0]","operator":"exists"}'
    ].join("\n"),
    emptyHelp: "留空时，所有已解析或增强的文档都会执行分块节点。"
  },
  enricher: {
    placeholder: [
      '分块存在：{"field":"chunks[0]","operator":"exists"}',
      '分块正文存在：{"field":"chunks[0].content","operator":"exists"}',
      '分块包含指定内容：{"field":"chunks[0].content","operator":"contains","value":"关键词"}'
    ].join("\n"),
    emptyHelp: "留空时，所有已生成的分块都会执行富集节点。"
  },
  indexer: {
    placeholder: [
      '分块向量存在：{"field":"chunks[0].embedding","operator":"exists"}',
      '分块摘要存在：{"field":"chunks[0].metadata[summary]","operator":"exists"}',
      '指定向量空间：{"field":"vectorSpaceId.logicalName","operator":"eq","value":"kb_default"}'
    ].join("\n"),
    emptyHelp: "留空时，所有已生成向量的分块都会执行索引节点。"
  }
};

const PARSER_RULES_PLACEHOLDER = `[
  {"mimeType":"PDF","options":{}},
  {"mimeType":"WORD","options":{}},
  {"mimeType":"ALL","options":{}}
]`;
  ...
  const buildNodeForm = (node: IngestionPipelineNode): PipelineNodeForm => {
    ...
    return {
      ...
      chunker: {
        ...
        rowsPerChunk: settings.rowsPerChunk != null ? String(settings.rowsPerChunk) : ""
      },
      enhancer: {
  ...
  const buildSettings = (node: PipelineNodeForm) => {
    switch (node.nodeType) {
      case "chunker": {
        ...
        const rowsPerChunk = node.chunker.rowsPerChunk.trim();
        const chunkSizeValue = chunkSize ? Number(chunkSize) : undefined;
        const overlapSizeValue = overlapSize ? Number(overlapSize) : undefined;
        const rowsPerChunkValue = rowsPerChunk ? Number(rowsPerChunk) : undefined;
        if (
          chunkSizeValue !== undefined &&
          (!Number.isInteger(chunkSizeValue) || (chunkSizeValue <= 0 && chunkSizeValue !== -1))
        ) {
          throw new Error("chunkSize 必须是大于 0 的整数，或填写 -1 表示整篇不分块");
        }
        if (
          overlapSizeValue !== undefined &&
          (!Number.isInteger(overlapSizeValue) || overlapSizeValue < 0)
        ) {
          throw new Error("overlapSize 必须是大于或等于 0 的整数");
        }
        if (rowsPerChunkValue !== undefined && (!Number.isInteger(rowsPerChunkValue) || rowsPerChunkValue <= 0)) {
          throw new Error("rowsPerChunk 必须是大于 0 的整数");
        }
        return {
          strategy: node.chunker.strategy,
          chunkSize: chunkSizeValue,
          overlapSize: overlapSizeValue,
          rowsPerChunk: rowsPerChunkValue
        };
      }
      case "enhancer": {
                ...
                {nodes.map((node, index) => (
                  <div key={node.id} className="rounded-lg border p-4 space-y-4">
                    ...
                    {node.nodeType === "parser" ? (
                      <div className="space-y-2">
                        <label htmlFor={`node-parser-rules-${node.id}`} className="text-sm font-medium">
                          解析规则（JSON，可选）
                        </label>
                        <Textarea
                          id={`node-parser-rules-${node.id}`}
                          aria-describedby={`node-parser-rules-help-${node.id}`}
                          rows={6}
                          ...
                          placeholder={PARSER_RULES_PLACEHOLDER}
                        />
                        <p
                          id={`node-parser-rules-help-${node.id}`}
                          className="text-xs leading-5 text-muted-foreground"
                        >
                          mimeType 可用：PDF、MARKDOWN、WORD、EXCEL、PPT、IMAGE、TEXT、ALL（全部）；留空允许所有类型。
                        </p>
                      </div>
                    ) : null}
                    ...
                    {node.nodeType === "chunker" ? (
                      <div className="space-y-2">
                        <div className="grid gap-4 md:grid-cols-2">
                          ...
                          <div className="space-y-2">
                            <label className="text-sm font-medium">目标分块大小（chunkSize）</label>
                            <Input
                              type="number"
                              ...
                              placeholder="例如：512；-1 表示整篇不分块"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-sm font-medium">重叠大小（overlapSize）</label>
                            <Input
                              type="number"
                              min={0}
                          ...
                          <div className="space-y-2">
                            <label className="text-sm font-medium">表格行数上限（rowsPerChunk）</label>
                            <Input
                              type="number"
                              min={1}
                              step={1}
                              value={node.chunker.rowsPerChunk}
                              onChange={(event) =>
                                setNodes((prev) =>
                                  prev.map((item) =>
                                    item.id === node.id
                                      ? { ...item, chunker: { ...item.chunker, rowsPerChunk: event.target.value } }
                                      : item
                                  )
                                )
                              }
                        ...
                        <p className="text-xs leading-5 text-muted-foreground">
                          默认使用 structure_aware；chunkSize 为 512、overlapSize 为 128、rowsPerChunk 为 50。
                        </p>
                      </div>
                    ) : null}
                    ...
                    {node.nodeType === "enhancer" ? (
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <label htmlFor={`node-enhancer-model-${node.id}`} className="text-sm font-medium">
                            模型ID
                          </label>
                          <Input
                            id={`node-enhancer-model-${node.id}`}
                            ...
                            placeholder="例如：qwen-flash、qwen-plus、qwen3-max；留空使用默认模型"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                              ...
                              <div key={task.id} className="rounded-md border p-3 space-y-3">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Textarea
                                    ...
                                    placeholder={
                                      "例如：请处理以下内容：{{content}}\n可用变量：{{text}}、{{content}}、{{mimeType}}、{{taskId}}、{{pipelineId}}"
                                    }
                                  />
                                </div>
                              </div>
                    ...
                    {node.nodeType === "enricher" ? (
                      <div className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            <label htmlFor={`node-enricher-model-${node.id}`} className="text-sm font-medium">
                              模型ID
                            </label>
                            <Input
                              id={`node-enricher-model-${node.id}`}
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              ...
                              <SelectContent>
                                <SelectItem value="true">是（附加）</SelectItem>
                                <SelectItem value="false">否（不附加）</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                        </div>
                              ...
                              <div key={task.id} className="rounded-md border p-3 space-y-3">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Textarea
                                    ...
                                    placeholder={
                                      "例如：请处理以下分块：{{content}}\n可用变量：{{text}}、{{content}}、{{chunkIndex}}、{{taskId}}、{{pipelineId}}"
                                    }
                                  />
                                </div>
                              </div>
                    ...
                    {node.nodeType === "indexer" ? (
                      <div className="space-y-2">
                        <label htmlFor={`node-indexer-metadata-${node.id}`} className="text-sm font-medium">
                          额外元数据字段
                        </label>
                        <Input
                          id={`node-indexer-metadata-${node.id}`}
                          aria-describedby={`node-indexer-metadata-help-${node.id}`}
                          ...
                          placeholder="例如：keywords, summary"
                        />
                        <p
                          id={`node-indexer-metadata-help-${node.id}`}
                          className="text-xs leading-5 text-muted-foreground"
                        >
                          用逗号分隔；留空时仍会保存 chunk_index、task_id、pipeline_id 和来源信息。
                        </p>
                      </div>
                    ) : null}

                    <div className="space-y-2">
                      <label htmlFor={`node-condition-${node.id}`} className="text-sm font-medium">
                        条件（JSON）
                      </label>
                      <Textarea
                        id={`node-condition-${node.id}`}
                        aria-describedby={`node-condition-help-${node.id}`}
                        rows={3}
                        ...
                        placeholder={NODE_CONDITION_GUIDES[node.nodeType].placeholder}
                      />
                      <div
                        id={`node-condition-help-${node.id}`}
                        className="rounded-md bg-muted/40 px-3 py-2 text-xs leading-5 text-muted-foreground"
                      >
                        <p>{NODE_CONDITION_GUIDES[node.nodeType].emptyHelp}</p>
                        <p>配置时只填写上方其中一条 JSON 示例，并按实际内容修改 value。</p>
                      </div>
                    </div>
                  </div>
                ))}
